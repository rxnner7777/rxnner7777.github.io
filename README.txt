...
## Новая логика названий и просмотров

Теперь название ролика и просмотры автоматически берутся с YouTube через YouTube Data API.

Когда добавляешь новую работу, тебе нужно указать только:
- ссылку на ролик
- путь к превью

Название в HTML можно оставить пустым или временным — сайт сам заменит его на актуальное название с YouTube.

Пример новой карточки:

```html
<article class="work-card" data-link="ССЫЛКА НА РОЛИК" data-img="assets/images/new-work.jpg">
  <button class="thumb-button" type="button" aria-label="Open thumbnail">
    <img src="assets/images/new-work.jpg" alt="">
  </button>
  <div class="title-row">
    <a class="video-title" href="ССЫЛКА НА РОЛИК" target="_blank" rel="noopener">Loading title...</a>
    <span class="views" data-video-url="ССЫЛКА НА РОЛИК" title="YouTube views"></span>
  </div>
</article>
```

Если API key не вставлен или лимит API закончился, сайт всё равно откроется, но автоматические названия/просмотры не обновятся.


## Обновление

Добавлены все новые работы из чата в категории СНГ и International.
Названия и просмотры подтягиваются с YouTube автоматически через YouTube Data API.
Карточки внутри каждой категории сортируются по просмотрам: самые популярные сверху.
